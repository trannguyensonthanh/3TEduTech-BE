...
const addCourseToCart = catchAsync(async (req, res) => {
  ...
  const updatedCart = await cartService.viewCart(accountId); // Lấy lại giỏ hàng sau khi thêm
  res
...
const removeCourseFromCart = catchAsync(async (req, res) => {
  ...
  const updatedCart = await cartService.viewCart(accountId); // Lấy lại giỏ hàng sau khi xóa
  res
    .status(httpStatus.OK)
